...
void initHw()
{
    ...
    if(controllerMode == 0)
    {
        // Configure UART1 pins driven to zero to start
        ...
        UART1_IBRD_R = 5;                                // r = 40 MHz / (Nx250kkHz), set floor(r)=21, where N=16
    ...
    else
    {
        ...
        GPIO_PORTC_AFSEL_R |= 0x10;                      // default, added for clarity
...
void timer1ISR()
{
    if(phase == 0)
    {
        ...
        GPIO_PORTC_DEN_R |= 0x60;
        TIMER1_CTL_R &= ~TIMER_CTL_TAEN;                 // turn-off timer before reconfiguring
        TIMER1_TAILR_R = 0x1E0;                         // set load value to 480 to interrupt every 12us
        phase = 1;
        TIMER1_CTL_R |= TIMER_CTL_TAEN;                  // turn-on timer
    }
    if(phase == 1)
...
void Uart1Isr()
{
    ...
    if(UART1_RIS_R & UART_RIS_TXRIS)
    {
        ...
        else
        {
            ...
            GPIO_PORTC_AFSEL_R &= ~0x60;                       // turn-off AFSEL for UART1
            GPIO_PORTC_DEN_R &= 0x60;
